...
export function ClipsGrid({ projectId }: { projectId: string }) {
  ...
  if (status === "error")
    return <div>Error loading clips: {(error as Error)?.message}</div>;

  const clips = flattenClips(data);

  if (!clips.length) {
    return (
      <div className="text-sm text-muted-foreground">No clips found yet.</div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 px-8 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {clips.map((clip) => (
          <div
            key={clip.id}
            className="rounded-2xl w-72 border border-muted bg-card"
          >
